...
- `topology`: 当前 Redis Cluster slot 拓扑，包括 Shard、Master、Replica、Redis nodeId 和 slot 分布；`observedNodes` 记录所有 managed Pod/Redis node inventory，包括 no-slot、ghost、dirty、deleting 节点
...
- **规划前等待稳定**：如果 live Redis 状态仍处在 failover 或 slot migrating/importing 等中间态，Controller 不调用 Planner，只设置 `WaitingForClusterStable` 并 requeue。等待超过 2 分钟后视为无法继续靠等待推进，放行到现有 Planner/Validator 修复流程。
...
- **删除发生在 activePlan 执行中**：当前 activePlan 已不再代表实际状态。Controller 应失败当前 Plan，保留完整 inventory 供人工处理；只有新的 CR generation 会将旧 Plan 标记为 `Superseded` 并触发重新规划。
...
第一版增量扩缩容支持 `ScaleOut` 和 `ScaleIn`：一次只能调整 `shards` 或 `replicasPerShard` 其中一个参数，不在同一个 Operation 中处理二者同时变更、镜像变更或内存规格变更。所有 Redis Pod 必须命名为 `redis-<N>`，其中 `<N>` 为非负整数；已进入 `status.topology`、`status.observedNodes` 或当前 plan allocation 的 Pod 名全局只能使用一次，Create 从 `redis-0` 开始创建初始连续序列，后续新增 Pod 必须使用全量 managed inventory 历史最大 ordinal 加 1，不填补删除空洞；Failed plan 不会自动释放 ordinal。Shard 扩容和缩容后的 slot 分布采用确定性的均衡策略，LLM 只能生成符合该策略的 `MigrateSlots` plan，Controller 必须在执行前复现并校验迁移矩阵。
