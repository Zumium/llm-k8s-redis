...
Controller 启动时从 ConfigMap 加载 LLM 配置，初始化 `planner.Client`，再把 client 交给 `LLMPlanner`。模型、`maxTokens`、`temperature`、`reasoningEffort` 都保存在 `OpenAIClient` 的 `Config` 里。`LLMPlanner` 不热更新；运行期要换模型、baseUrl 或 apiKey，就更新 ConfigMap 后重启 controller。
...
- `CompletionRequest.System` 和 `CompletionRequest.Prompt` 固定翻译成 system/user 两条消息。
