...
| `ForgetNode` | `namespace` string, optional `pod` string, optional `lastKnownNodeId` string |
...
`ForgetNode` 必须提供 `pod` 或 `lastKnownNodeId` 之一；当目标 Pod 已不存在，或 Redis ghost 无法映射回 Pod 名时，必须携带 `lastKnownNodeId`。`DeleteNode` 仍必须携带真实 `pod`。`slots` 必须能被 controller 的 slot parser 解析，且每个 slot 都在 `0-16383` 范围内。
...
| --- | --- |
| `EnsureNode` | 确保 Pod 存在；校验 `image` / `memorySize` 等于 spec；可删除未入群的 spec 漂移 Pod 让其重建；不加入 Redis Cluster，不设置 master/replica，不分配 slots。 |
...
| `MigrateSlots` | 要求 source/target 都是 ready healthy master，且 target 已有至少一个 replica；source 和 target 不能相同；目标 slots 当前 owner 只能是 source 或 target，暂无 owner 的目标 slots 可补到 target；迁移后 owner 变为 target。 |
