...
func TestMigrateSlots_UnrelatedReadyPodDoesNotBlockPreflight(t *testing.T) {
	ctx := context.Background()
	cluster := testCluster()
	migrateTopology(cluster)
	var fc *fakeRedisClient
	fc = &fakeRedisClient{
		clusterNodes: func() (string, error) {
			fc.mu.Lock()
	...
	exec.RedisFactory = func(addr string) (redis.Client, error) {
		if addr == "10.0.0.5:6379" {
			return nil, errors.New("unrelated pod should not be contacted")
		}
		return &addrFakeRedisClient{fakeRedisClient: fc, addr: addr}, nil
	}
	pod := readyPodWithIP(cluster, "redis-4", "10.0.0.5")
	if err := ctrl.SetControllerReference(cluster, pod, exec.Scheme); err != nil {
		t.Fatalf("set owner ref: %v", err)
	}
	if err := exec.Create(ctx, pod); err != nil {
...
func TestMigrateSlots_RedisSeenPeerTargetGossipMismatchReturnsRunning(t *testing.T) {
	ctx := context.Background()
	cluster := testCluster()
	migrateTopology(cluster)
	third := "third333 10.0.0.5:6379@16379 master - 0 0 5 connected\n"
	fc := &fakeRedisClient{
		clusterNodesAddr: func(addr string) (string, error) {
			if addr == "10.0.0.5:6379" {
				return migrateSourceID + " 10.0.0.1:6379@16379 master - 0 0 1 connected 0-8191\n" +
					migrateReplica1 + " 10.0.0.2:6379@16379 slave " + migrateSourceID + " 0 0 2 connected\n" +
					migrateTargetID + " 10.0.0.3:6379@16379 slave " + migrateSourceID + " 0 0 3 connected\n" +
					migrateReplica2 + " 10.0.0.4:6379@16379 slave " + migrateTargetID + " 0 0 4 connected\n" +
					third, nil
			}
			return migrateClusterNodes("0-8191", "8192-16383") + third, nil
...
func TestMigrateSlots_SetSlotMasterGossipErrorReturnsRunning(t *testing.T) {
	ctx := context.Background()
	cluster := testCluster()
	migrateTopology(cluster)
	fc := &fakeRedisClient{
		...
		setSlotNode: func(slot int, nodeID string) error {
			return errors.New("ERR Please use SETSLOT only with masters")
		},
	}
	exec := migrateExec(t, cluster, fc)
