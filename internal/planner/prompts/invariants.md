...
- Heal repair (spec.shards matches observed topology and all 16384 slots are covered, but some shard has fewer than spec.replicasPerShard replicas): only EnsureNode, WaitNodeReady, MeetNode, ReplicateNode, ForgetNode, DeleteNode, VerifyCluster are allowed; a ghost node (redisSeen=true, owns no slots, and pod gone/deleting, fail/fail?/handshake/noaddr, or linkState=disconnected) must be forgotten by a ForgetNode step carrying "lastKnownNodeId" equal to the ghost's nodeId. Ready=false alone is ambiguous and must not be treated as a ghost. The pod field is only the last known Pod name and must not be the sole ghost identity.
